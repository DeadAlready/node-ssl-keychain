...
KeyChain.prototype.init = function(dontCreate, noErrors, callback){
  ...
  self.clearFolders(function(){
    foldermap.mapTree(self.root + self.folder, self._cC(callback, function(map){
      self[self.folder] = map;
      self.checkAllFiles(dontCreate, noErrors, self._cC(callback));
    }));
  });
}
...
KeyChain.prototype.mapFolderSync = function(){
  ...
  self._map = foldermap.mapTreeSync(utils.fixPath(self.root) + self.folder);
  self.certs = {};
...
KeyChain.prototype._reMap = function(callback, created){
  ...
  foldermap.mapTree(utils.fixPath(self.root) + self.folder, self._cC(callback, function(map){
    self._map = map;
    self.certs = {};
    for(var i in self.certInfo){
      self.certs[i] = {};
...
KeyChain.prototype._returnMap = function _returnMap(callback, name, created, info){
  ...
  foldermap.mapTree(self._getIncomingFilePath(name, info), function(err, map){
    ...
    if(info !== 'incoming'){
      ...
      self.certs[info][name][path.extname.substr(1)] = map;
    }
    callback(null, map);
  });
}