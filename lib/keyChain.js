...
KeyChain.prototype.checkAllFiles = function(dontCreate, noErrors, callback){
  ...
  for(var i in self.certInfo){
    ...
    if(info.required){
      errors = errors.concat(self._checkFiles(map, info));
      continue;
    }
    if(info.create){
      if(self._checkFiles(map, info).length > 0){
        create.push(info);
      }
      continue;
    }
    self._checkFiles(map, info);
  }
 ...
 */
KeyChain.prototype._checkFiles = function(map, info){
  this.log('debug','_checkFiles');
  var self = this;
  var errors = [];
  if(info.folder === ''){
